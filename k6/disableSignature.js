...
async function submitWithoutEncrypt(publicKey, secretString, data, seq) {
...
(async () => {
  try {
    var accountL = JSON.parse(fs.readFileSync('accountList.json', 'utf8'))[
      'users'
    ];
    console.log('number of users : ', accountL.length);
    const num_account = accountL.length;
    const all_key = (num_account * num_payload * record_len) / 63;
    let i = 0;
    let total = 0;
    while (i < num_account) {
      let j = 0;
      ...
      const seq = account.sequenceNumber() - 3;
      while (j < num_payload) {
